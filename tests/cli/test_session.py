"""Compiler calls work without a login; catalogue credentials stay private and required."""
...
@pytest.mark.parametrize("configured_origin", [
    None, "https://157.180.79.112", "http://127.0.0.1:18765",
])
@pytest.mark.parametrize("arguments, endpoint", [
...
def test_compiler_commands_need_no_login(
    ...
    (tmp_path / "project.json").write_text(
        json.dumps(blank_graph("test.project", "Anonymous compiler")), encoding="utf-8"
    )
    ...
    if configured_origin is not None:
        ...
        monkeypatch.setattr("sys.stdin", object())  # Compiler calls never consume a token.
    received: list[str] = []
...
def test_compiler_fallback_preserves_configured_service_and_catalogue_auth(
    ...
    assert credential("https://project.test", anonymous=True).origin == "https://saved.test"
    forget()
    (tmp_path / ".git").mkdir()
    (tmp_path / "project.json").write_text(
        json.dumps(blank_graph("test.project", "Catalogue auth")), encoding="utf-8"
    )
    monkeypatch.chdir(tmp_path)
...
@pytest.mark.parametrize("origin", [
    "https://verdog.test", "http://127.0.0.1:8765",
...
@pytest.mark.parametrize("origin", [
    "http://verdog.test", "http://localhost.evil", "http://127.0.0.1.evil",
    "https://user:password@verdog.test", "https://verdog.test/path",
    "https://verdog.test?secret", "https://verdog.test#fragment",
    "https://verdog.test:", "https://verdog.test:invalid",
    "https://verdog.test:65536", "https://verdog.test:0",
    "https:///missing-host", "file:///tmp/verdog", "https://verdog.test\n",
])
def test_login_rejects_unsafe_origins_before_reading_the_token(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str], origin: str
) -> None:
    monkeypatch.setattr("sys.stdin", object())  # Has no read method: it must not be touched.
