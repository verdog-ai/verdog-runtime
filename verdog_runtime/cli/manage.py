"""Manage credentials, catalogue offers, and pinned workflow dependencies."""
...
from verdog_runtime.cli import api, local, session
from verdog_runtime.cli import catalogue as cli_catalogue
...
def _scoped_subroutine_definitions(
    ...
    def visit(definition: dict[str, Any], owner: str | None) -> None:
        identifier = local.definition_child(
            owner, str(definition.get("id", ""))
        )
...
def _subroutine_definitions(project: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        definition for _, definition in _scoped_subroutine_definitions(project)
    ]
...
def _rows(
    values: list[dict[str, Any]], columns: tuple[tuple[str, str], ...]
) -> None:
    ...
    print(
        "  ".join(
            title.ljust(width)
            for (title, _), width in zip(columns, widths, strict=True)
        )
    )
    for row in values:
        print(
...
def _client(clone: local.Clone | None = None) -> api.Service:
    """Use the interactive session when present, otherwise the clone token."""
...
def _here() -> local.Clone:
    return local.open_clone(pathlib.Path.cwd())


def _login(arguments: argparse.Namespace) -> int:
    """Exchange a GitHub token from the terminal or stdin."""
...
def _logout(arguments: argparse.Namespace) -> int:
    ...
    try:
        ...
        # The local copy goes either way: a session the service already forgot
        # is not a
...
def _whoami(arguments: argparse.Namespace) -> int:
    ...
    print(
        f"{user.get('login') or user.get('email') or user.get('id')} at "
        f"{client.origin}"
    )
...
def _clone(arguments: argparse.Namespace) -> int:
    """Clone a GitHub repository, initialize pins, and set its service.

    The whole of what cloning used to do, minus everything Verdog was doing
    itself: there is no bundle to unpack, no pins to materialize one at a
    time, and no token to store -- submodule urls are real GitHub urls now,
    ...
    if destination.exists():
        raise local.WorkspaceError(f"{destination} already exists")
    https_url = f"https://github.com/{address}.git"
    ssh_url = f"git@github.com:{address}.git"
    if arguments.ssh:
        url = ssh_url
    else:
        access = local.git_with_ssh_fallback(
            pathlib.Path.cwd(),
            "ls-remote",
            "--quiet",
            https_url,
    ...
    local.git(pathlib.Path.cwd(), "clone", "--quiet", url, str(destination))
    local.git_with_ssh_fallback(
        destination,
        "submodule",
        "update",
        "--init",
...
The service uses the same root id. Integration tests run this client's
output
...
def package_from(name: str) -> str:
    ...
    Lowercase snake_case, because that is what the compiler accepts and what
    a Python package has to be anyway. Not clever: it is offered back to the
    author, who can override it.
    """
...
def _remote_owner(root: pathlib.Path) -> str | None:
    """The owner of this directory's GitHub remote, if it has one.

    Only a suggestion for the space, and read with `_quiet_git` because "no
    remote" is the ordinary case rather than a failure: `verdog init` works
    in a directory that is not a repository yet, and a project without a
    remote is a project that has not been published.
    """
    ...
    if remote is None:
        return None
    try:
        owner, _ = local.repository_address(remote.strip())
    except local.WorkspaceError:
        return None
    return owner


def blank_graph(package: str, name: str) -> dict[str, Any]:
    """Create a minimal workflow graph with one edge.

    Everything else -- every module, every declaration, `pyproject.toml`,
    the contract scaffolds -- is produced by the ordinary projection on the
    first `verdog generate`. There is no separate "new project" template
    that could drift from what the generator emits, which is the whole
    reason this is a graph and not a directory of files.

    `generated_from` is a hash no graph can have, which is how a project
    that has never been generated says so. The enter->exit edge is not
    optional: without it the generator refuses the graph, because an enter
    port with no outgoing edge cannot start anything.
    """
...
`verdog run` writes into `.verdog/`; editable installs create `*.egg-info/`.
These are not project content. The next thing an author does is
...
def _init(arguments: argparse.Namespace) -> int:
    ...
    Writes the graph, then runs `generate` -- which is what turns one file
    ...
    if not arguments.package and owner is None:
        raise local.WorkspaceError(
            "init requires `--package <space>.<name>` "
            "because no GitHub remote provides a space"
        )
    package = str(
        arguments.package or f"{package_from(owner or '')}.{package_from(name)}"
    )
    complaint = local.package_problem(package)
    if complaint is not None:
        raise local.WorkspaceError(
            f"{package!r} cannot be a package name: {complaint}"
        )
    ...
    # Git, because every other verb finds the project by walking up to a `.git`.
    # A remote is
    # not needed and not invented: the service accepts files; only publishing
    # needs a remote.
    if not (root / ".git").exists():
        local.git(root, "init", "--quiet")
    ...
    from verdog_runtime.cli import main

    status = main.generate_clone(local.open_clone(root))
    if status == 0:
...
def _access(arguments: argparse.Namespace) -> int:
    """Report current catalogue permissions."""
    clone = _here()
    ...
    if not answer.get("accessible", True):
        print(
            f"{answer.get('repository')}: your GitHub token cannot access this "
            f"repository."
        )
        return 0
...
def _catalogue(arguments: argparse.Namespace) -> int:
    ...
    if entry_id:
        entry = session.account().entry(str(entry_id))
        if getattr(arguments, "as_json", False):
            print(json.dumps(entry, indent=2))
        else:
            _print_catalogue_entry(entry)
        return 0
    answer = session.account().catalogue(
        ...
        [
            {
                ...
                "reference": f"{entry.get('repository')} · "
                f"{entry.get('workflow_id')}",
...
def _print_catalogue_entry(entry: dict[str, Any]) -> None:
    ...
    requirement_count = (
        len(cast(list[Any], requirements))
...
def _release_column(entry: dict[str, object]) -> str:
    ...
    The listing shows one row per workflow rather than one per commit, so a
    reader who is told only the newest hash has no way to know the others
    exist. `--json` carries them.
    """
    ...
    releases = (
        ...
        else len(_objects(entry.get("releases")))
    )
    older = releases - 1
...
def publish_workflow(arguments: argparse.Namespace) -> int:
    ...
    `HEAD`, and it has to be pushed: an offer names a commit somebody else
    will fetch from GitHub, so publishing a commit that exists only on this
    machine would advertise something nobody can get.

    And it has to *work*. A published offer is a claim made to strangers;
    the least it can do is be a claim about a project that compiles.
    """
    clone = _here()
    owner, name = clone.require_repository()
    description = str(arguments.description).strip()
    if not description:
        raise local.WorkspaceError("--description must not be empty")
    # Deferred, the way `_init` imports it: `main` imports this module, so the
    # cycle is real.

    from verdog_runtime.cli import main

    status, _ = main.check_clone_result(clone)
    if status != 0:
        raise local.WorkspaceError(
            "this project does not pass `verdog check`, so "
            "there is nothing worth offering. "
            "Fix the problems above and publish the commit that fixes them."
        )
    # `check` writes the generated tree, so a clean check can leave the working
    # copy dirty --
    # and then `HEAD` is not what was just verified. An offer names a commit,
    # not a worktree.
    if local.git(clone.root, "status", "--porcelain").strip():
        raise local.WorkspaceError(
            "the working copy has changes that HEAD does "
            "not, so an offer at HEAD would "
    ...
    descriptor = cli_catalogue.describe_workflow(clone, workflow)
    if arguments.private:
        print(
            "verdog: --private is deprecated; catalogue "
            "visibility follows the GitHub "
            "repository.",
            file=sys.stderr,
        )
...
def _retract(arguments: argparse.Namespace) -> int:
    ...
    By id and not by address, because an offer names a commit: a repository
    can have several live releases of one workflow and withdrawing "the
    workflow" is not a thing you can mean. `catalogue --json` carries the id
    of every release, which is where one comes from.

    What this does not do is withdraw code. A commit somebody already
    fetched stays fetched and their pin keeps working -- only the host can
    make a commit unreachable. Retracting says "I no longer offer this for
    reuse", and nothing stronger.
    """
    entry_id = str(arguments.entry)
    session.account().retract(entry_id)
    print(f"Retracted catalogue entry {entry_id}.")
    print(
        "  Anyone who already imported it keeps working: "
        "a fetched commit stays fetched."
    )
    return 0


def clone_head(clone: local.Clone, repository: str | None = None) -> str:
    """Require HEAD on a tracking branch of the repository being advertised."""
    commit = local.git(clone.root, "rev-parse", "HEAD").strip()
...
def _repository_remotes(clone: local.Clone, repository: str) -> tuple[str, ...]:
    """Find conventional remotes matching the selected GitHub repository."""
    found: list[str] = []
    for remote in ("origin", "upstream"):
        try:
            url = local.git(clone.root, "remote", "get-url", remote).strip()
            address = "/".join(local.repository_address(url))
        except local.WorkspaceError:
            continue
...
def offer_for(clone: local.Clone, workflow_id: str) -> dict[str, Any]:
    """The graph preview published for a workflow.

    This is the publisher's claim, and nothing the service went and looked
    for. It carries enough of the graph to browse before importing instead
    of after.

    `editor` is deliberately absent: a preview lays itself out, and a layout
    is not part of what a workflow *is*.
    """
    try:
        return {"preview": cli_catalogue.preview_for(clone, workflow_id)}
    except local.WorkspaceError as error:
        if "unknown workflow" not in str(error):
            raise
        raise local.WorkspaceError(
            f"this project has no workflow {workflow_id}"
        ) from error


def _init_submodules(checkout: pathlib.Path) -> None:
    """Initialize every dependency owned by one freshly selected checkout."""
    local.git(checkout, "submodule", "sync", "--recursive")
    local.git_with_ssh_fallback(
        checkout,
        "submodule",
        "update",
...
def _external_checkout(clone: local.Clone, alias: str) -> pathlib.Path:
    """An alias path proven to stay inside this clone's `external/` tree."""
    relative = pathlib.Path(local.external_root(alias))
    root = clone.root.resolve()
    external = (clone.root / "external").resolve()
    checkout = local.contained_path(clone.root, relative)
    ...
    for ancestor in checkout.parents:
        ...
        if metadata.exists() or metadata.is_symlink():
            raise local.WorkspaceError(
                f"external alias "
                f"{alias!r} is nested inside another Git worktree"
            )
    return clone.root / relative


def _node_directory(
    clone: local.Clone, source: pathlib.Path, node_id: str
) -> pathlib.Path:
    """A node path proven safe before recursive removal."""
    if source.is_absolute() or not source.parts or ".." in source.parts:
        raise local.WorkspaceError(
            f"definition path {source} escapes this project"
        )
    if (
        not node_id
        or node_id in {".", ".."}
        or pathlib.Path(node_id).parts != (node_id,)
    ):
        raise local.WorkspaceError(
            f"node id {node_id!r} is not a safe path component"
        )
    relative = source / "nodes" / node_id
    return local.contained_path(clone.root, relative)
...
def _visit_implementation(
    ...
    for label, value in (("target node id", target_id), ("edge id", edge_id)):
        if (
            not value
            or value in {".", ".."}
            or pathlib.Path(value).parts != (value,)
        ):
            raise local.WorkspaceError(
    ...
    relative = (
        node.relative_to(clone.root.resolve()) / "visit" / edge_id / "impl.py"
    )
    return local.contained_path(clone.root, relative)


def _module_git_dir(clone: local.Clone, target: str) -> pathlib.Path:
    raw = local.git(
        clone.root, "rev-parse", "--git-path", f"modules/{target}"
    ).strip()
    path = pathlib.Path(raw)
    return path if path.is_absolute() else clone.root / path


def _reuse_recovery(clone: local.Clone, target: str, repository: str) -> bool:
    """Reuse a dropped submodule's objects only for the same repository."""
    module = _module_git_dir(clone, target)
    if not module.exists():
        return False
    if module.is_symlink() or not module.is_dir():
        raise local.WorkspaceError(
            f"Git recovery data for {target} is not a directory"
        )
    ...
    if remote is None:
        raise local.WorkspaceError(
            f"Git recovery data for {target} has no readable origin"
        )
    try:
        recovered = local.repository_address(remote.strip())
    except local.WorkspaceError as error:
        raise local.WorkspaceError(
            f"Git recovery data for "
            f"{target} has an unrecognised origin; use another alias"
        ) from error
    if recovered != local.repository_address(repository):
        raise local.WorkspaceError(
            f"Git recovery data for "
            f"{target} belongs to "
            f"{recovered[0]}/"
...
def _rollback_import(
    ...
    """Undo a failed import while retaining its recovery object store."""
    errors: list[str] = []
...
def _rollback_bump(
    ...
    if not _quiet_git(target, "submodule", "deinit", "--force", "--all"):
        errors.append(
            f"could not deinitialize {target_name}'s nested dependencies"
        )
    checkout_restored = False
    try:
        local.git(target, "checkout", "--force", "--quiet", "--detach", head)
...
def _report_rollback(errors: list[str]) -> None:
    if errors:
        print(
            "verdog: rollback incomplete: " + "; ".join(errors), file=sys.stderr
        )
...
def _import(arguments: argparse.Namespace) -> int:
    if not getattr(arguments, "as_json", False):
        ...
        return status
    # Git and generation retain useful progress on stderr; stdout is one
    # protocol object.
...
def _import_result(arguments: argparse.Namespace) -> tuple[int, dict[str, Any]]:
    ...
    Four steps, and only the first is ours: pre-flight the offer, `git
    submodule add` at the commit, write the `externals` entry and a workflow
    binding, then `check`. The fetch uses your credentials, so a repository
    you cannot clone is one you cannot import -- there is no path through
    this service for the bytes.
    """
    ...
    status = main.generate_clone(plan.clone)
...
def _prepare_import(arguments: argparse.Namespace) -> _ImportPlan:
    ...
    owner, name, repository, commit = _import_reference(
    ...
    alias, target = _import_alias(
        project, getattr(arguments, "alias", None), package
    )
    closure = _objects(entry.get("closure"))
    _print_import_offer(repository, commit, workflow, package, closure)
    checkout = _external_checkout(clone, alias)
    if checkout.exists():
        raise local.WorkspaceError(f"{target} already exists")
    return _ImportPlan(
        clone,
        project,
        parent,
...
def _import_parent(project: dict[str, Any], requested: object) -> str:
    ...
    known = {
        identifier for identifier, _ in _scoped_subroutine_definitions(project)
    }
    if parent not in known:
        raise local.WorkspaceError(f"this project has no subroutine {parent}")
    return parent
...
def _import_reference(address: str) -> tuple[str, str, str, str]:
    ...
    if not separator or re.fullmatch(r"[0-9a-fA-F]{40}", commit) is None:
        raise local.WorkspaceError(
            "give the release as owner/name@<40-character "
            "commit>, or a GitHub url with "
            "the commit appended the same way"
        )
    owner, name = local.repository_address(reference)
...
def _import_package(entry: dict[str, Any], repository: str, commit: str) -> str:
    ...
    if complaint:
        raise local.WorkspaceError(
            f"{repository}@"
            f"{commit[:12]} publishes an invalid package "
            f"{package!r}: "
            f"{complaint}"
        )
    return package
...
def _import_alias(
    ...
    complaint = local.package_problem(alias)
    if complaint:
        raise local.WorkspaceError(
            f"{alias!r} cannot be an external alias: {complaint}"
        )
    pins = _objects(project.get("externals"))
    if any(pin.get("alias") == alias for pin in pins):
        raise local.WorkspaceError(
            f"this project already pins the alias {alias!r}"
        )
    target = local.external_root(alias)
    for pin in pins:
        other = local.external_root(str(pin.get("alias", "")))
        if target.startswith(other + "/") or other.startswith(target + "/"):
            raise local.WorkspaceError(
                f"external alias "
                f"{alias!r} overlaps "
                f"{pin.get('alias')!r} on disk"
            )
    return alias, target
...
def _gitmodules_snapshot(
    plan: _ImportPlan,
) -> tuple[pathlib.Path, bytes | None, tuple[str, str] | None]:
    modules = plan.clone.root / ".gitmodules"
    if modules.is_symlink() or (modules.exists() and not modules.is_file()):
        raise local.WorkspaceError(".gitmodules is not a regular file")
    listed = _quiet_git_output(
        plan.clone.root, "ls-files", "--stage", "--", ".gitmodules"
    )
    if listed is None:
        raise local.WorkspaceError("Git could not inspect .gitmodules")
    entries = [line.partition("\t")[0].split() for line in listed.splitlines()]
    if any(len(fields) != 3 or fields[2] != "0" for fields in entries):
        raise local.WorkspaceError(".gitmodules has unresolved index entries")
    if len(entries) > 1:
        raise local.WorkspaceError(".gitmodules has more than one index entry")
    ...
    if not modules.exists() and (
        ...
        raise local.WorkspaceError(
            ".gitmodules is missing from the working tree; "
            "restore it deliberately before "
            "importing"
        )
...
def _require_unused_import_target(plan: _ImportPlan) -> None:
    ...
    if indexed is None:
        raise local.WorkspaceError(
            f"Git could not inspect {plan.target} in the index"
        )
    if indexed.strip():
        raise local.WorkspaceError(
            f"{plan.target} is already present in the Git index"
        )
...
def _materialize_import(plan: _ImportPlan, reuse: bool) -> str:
    ...
    local.git(plan.checkout, "checkout", "--quiet", "--detach", plan.commit)
    _verify_import_checkout(plan)
    _init_submodules(plan.checkout)
    local.git(plan.clone.root, "add", "--", plan.target)
...
def _verify_import_checkout(plan: _ImportPlan) -> None:
    imported = local.Clone(plan.checkout, plan.clone.origin, None)
    package = imported.project.get("package")
    if package != plan.package:
        raise local.WorkspaceError(
            f"{plan.repository}@"
            f"{plan.commit[:12]} contains package "
            f"{package!r}, "
    ...
    try:
        ...
        raise local.WorkspaceError(
            f"{plan.repository}@"
            f"{plan.commit[:12]} has no workflow "
            f"{plan.workflow!r}"
        ) from error


def _remote_package(repository: str, commit: str) -> str:
    """Read release metadata without changing the consumer clone.

    The service deliberately cannot read repository content, so its
    authorized unpublished preflight has no package. A short-lived bare
    repository obtains exactly `project.json`; the final submodule fetch
    still performs the ordinary import with the user's Git setup.
    """
    with tempfile.TemporaryDirectory(
        prefix="verdog-import-metadata-"
    ) as temporary:
        metadata_root = pathlib.Path(temporary)
        local.git(metadata_root, "init", "--quiet", "--bare")
        local.git_with_ssh_fallback(
            metadata_root,
            "fetch",
            "--quiet",
        ...
        try:
            decoded = json.loads(
                local.git(metadata_root, "show", f"{commit}:project.json")
            )
...
def bump_dependency(arguments: argparse.Namespace) -> int:
    """Advance a dependency pin without changing its calling graph.

    Validate the requested manifest before changing the checkout. If checkout or
    manifest updates fail, restore the previous commit and manifest.
    """
    clone = _here()
    project = clone.project
    ...
    pins = {
        str(pin.get("alias")): pin for pin in _objects(project.get("externals"))
    }
    pin = pins.get(alias)
    if pin is None:
    ...
    target_name = local.external_root(alias)
    ...
    if not (target / "project.json").is_file():
        raise local.WorkspaceError(
            f"{target_name} has no readable project.json"
        )
    top_level = pathlib.Path(
        local.git(target, "rev-parse", "--show-toplevel").strip()
    ).resolve()
    if top_level != target.resolve():
        raise local.WorkspaceError(f"{target_name} is not its own Git worktree")

    listed = local.git(clone.root, "ls-files", "--stage", "--", target_name)
    ...
    if (
        ...
        raise local.WorkspaceError(
            f"the owner does not record "
            f"{target_name} as one exact, unconflicted gitlink"
        )
    mode, indexed_commit, _ = entries[0][0]
    head = local.git(target, "rev-parse", "HEAD").strip()
    recorded_commit = str(pin.get("commit", ""))
    if recorded_commit != indexed_commit:
        raise local.WorkspaceError(
            f"project.json records "
            f"{recorded_commit[:12] or 'no commit'} for "
            f"{alias}, but "
            f"its owner records {indexed_commit[:12]}"
        )
    # Refuse before fetching, not after: bumping over uncommitted work in a
    # dependency would
    ...
    if local.git(
        ...
        raise local.WorkspaceError(
            f"{target_name} has uncommitted changes. Commit and push them, "
            f"record the commit, "
            "then restore the owner-recorded checkout before "
            "bumping with `--commit`."
        )
    ...
    if re.fullmatch(r"[0-9a-fA-F]{40}", commit) is None:
        raise local.WorkspaceError("give the commit as a full 40-character id")
    commit = commit.lower()
    if head not in {indexed_commit, commit}:
        raise local.WorkspaceError(
            f"{target_name} is at {head[:12]}, but its owner records "
            f"{indexed_commit[:12]} and this bump requests "
            f"{commit[:12]}. Restore one of those "
    ...
    # The same pre-flight an import does, and for the same reason: a commit
    # nobody published is
    # one nobody else can fetch, so a pin naming it is a project only this
    # machine can build.
    entry = _client(clone).preflight(repository, commit, workflow)
    if str(entry.get("package", package)) != package:
        raise local.WorkspaceError(
            f"{repository}@"
            f"{commit[:12]} publishes "
            f"{entry.get('package')!r}, not "
            f"{package!r}. A "
    ...
    try:
        decoded = json.loads(
            local.git(target, "show", f"{commit}:project.json")
        )
        if not isinstance(decoded, dict):
    ...
    head_reference = (
        head_reference.strip() if head_reference is not None else None
    )
    try:
        local.git(target, "checkout", "--quiet", "--detach", commit)
        _init_submodules(target)
        local.git(clone.root, "add", "--", target_name)
        updated = {
            **pin,
            "commit": commit,
            "repository_id": entry.get(
                "repository_id", pin.get("repository_id")
            ),
        }
        project["externals"] = [
    ...
    # A dependency may change qualified calls in its parent, so regenerate it
    # through the service.
    from verdog_runtime.cli import main

    return main.generate_clone(clone)


def _pinned_workflow(clone: local.Clone, alias: str) -> str:
    ...
    raise local.WorkspaceError(
        f"nothing in this graph binds "
        f"{alias}, so there is no release to look up. "
        "Import its workflow or pass --commit."
    )


def _newest_release(clone: local.Clone, repository: str, workflow: str) -> str:
    """The newest commit the catalogue offers for this workflow.

    Search narrows the catalogue, but exact identity still decides:
    descriptions can contain repository names, and the requested workflow
    may sit beyond the first result page.
    """
    ...
    while True:
        ...
        if (
            ...
            raise local.WorkspaceError(
                "the catalogue returned an invalid pagination cursor"
            )
        seen.add(next_cursor)
        cursor = next_cursor
    raise local.WorkspaceError(
        f"the catalogue offers no release of "
        f"{workflow} from "
        f"{repository}. Publish one, or pass "
        "--commit to pin a specific one."
    )


def _quiet_git_output(root: pathlib.Path, *arguments: str) -> str | None:
...
def _quiet_git(root: pathlib.Path, *arguments: str) -> bool:
    """Run git where failure is an expected answer, not an error.

    `config --remove-section` exits non-zero when the section is not there,
    which is the ordinary case when a previous drop already removed it.
    `git()` raises, and rightly -- so the few places that are asking a
    question rather than giving an order use this instead.
    """
    return (
...
def _validate_drop_checkout(
    checkout: pathlib.Path, target: str, indexed_commit: str
) -> None:
    """Refuse to discard uncommitted work or an unrecorded child commit."""
    ...
    if initialized and dirty is None:
        raise local.WorkspaceError(
            f"{target} is initialized but Git could not inspect it"
        )
    ...
    head = (
        _quiet_git_output(checkout, "rev-parse", "HEAD")
        if initialized
        else None
    )
    if initialized and head is None:
        raise local.WorkspaceError(
            f"{target} is initialized but Git could not read its HEAD"
        )
...
def _calls_dependency(
    node: dict[str, Any], alias: str, binding_ids: set[str]
) -> bool:
    target = _object(node.get("operation")).get("target")
    if node.get("kind") == "workflow_call":
        return target in binding_ids
    return (
...
def _dependency_gitlinks(root: pathlib.Path, target: str) -> dict[str, str]:
    """Read exact and nested gitlinks, tolerating an absent index entry."""
    indexed = _quiet_git_output(root, "ls-files", "--stage", "--", target)
    links: dict[str, str] = {}
    for line in (indexed or "").splitlines():
        metadata, separator, path = line.partition("\t")
...
def _dependency_sections(root: pathlib.Path, target: str) -> list[str]:
    configured = _quiet_git_output(
        root,
        "config",
        "--file",
    ...
    sections: list[str] = []
    for line in (configured or "").splitlines():
...
def _removed_call_visits(
    clone: local.Clone,
    source: pathlib.Path,
    definition: dict[str, Any],
    removed_ids: set[str],
) -> set[pathlib.Path]:
    """Find visits from removed callers into surviving, executable nodes."""
    nodes = {
        str(node.get("id", "")): node
        for node in _objects(definition.get("nodes"))
    }
    visits: set[pathlib.Path] = set()
    for edge in _objects(definition.get("edges")):
        source_id = str(edge.get("source", ""))
        target_id = str(edge.get("target", ""))
        if source_id not in removed_ids or target_id in removed_ids:
            continue
        target = nodes.get(target_id)
        if target is None or target.get("kind") in {"enter", "exit", "failure"}:
            continue
        visits.add(
            _visit_implementation(
                clone, source, target_id, str(edge.get("id", ""))
            )
        )
    return visits


def _remove_dependency_calls(
    clone: local.Clone,
    project: dict[str, Any],
    alias: str,
    binding_ids: set[str],
) -> tuple[int, int, set[pathlib.Path], set[pathlib.Path]]:
    """Edit the in-memory graph; return counts and authored paths to remove."""
    bindings = callers = 0
    ...
    for definition_id, definition in _scoped_subroutine_definitions(project):
        ...
        bindings += len(children) - len(kept)
        definition["workflows"] = kept
        nodes = _objects(definition.get("nodes"))
        removed_ids = {
            str(node.get("id", ""))
            for node in nodes
            if _calls_dependency(node, alias, binding_ids)
        }
        remaining = [
            node
            for node in nodes
            if not _calls_dependency(node, alias, binding_ids)
        ]
        callers += len(nodes) - len(remaining)
        if len(nodes) == len(remaining):
            continue
        source = sources.get(definition_id)
        if source is not None:
            directories.update(
                _node_directory(clone, source, node_id)
                for node_id in removed_ids
            )
            visits.update(
                _removed_call_visits(clone, source, definition, removed_ids)
            )
        definition["nodes"] = remaining
        names = {str(node.get("id")) for node in remaining}
        definition["edges"] = [
            edge
            for edge in _objects(definition.get("edges"))
            if str(edge.get("source")) in names
            and str(edge.get("target")) in names
        ]
    return bindings, callers, directories, visits


def _drop_authored_code(
    clone: local.Clone,
    project: dict[str, Any],
    directories: set[pathlib.Path],
    visits: set[pathlib.Path],
    *,
    discard: bool,
) -> None:
    """Require consent before removing authored code and source entries."""
    prefixes = {
        directory.relative_to(clone.root).as_posix() + "/"
        for directory in directories
    }
    visit_paths = {file.relative_to(clone.root).as_posix() for file in visits}
    tracked = {
        str(source.get("path", ""))
        for source in _objects(project.get("sources"))
    }
    affected = {
        path.relative_to(clone.root).as_posix()
        for path in directories | visits
        if path.exists()
    }
    affected.update(
        prefix
    ...
    affected.update(visit_paths & tracked)
    if affected and not discard:
        listed = "\n".join(f"  {path}" for path in sorted(affected))
        raise local.WorkspaceError(
            "dropping this dependency removes authored node code and visits; "
            f"rerun with --discard-node-code to confirm:\n{listed}"
        )
    for directory in directories:
        if directory.exists():
    ...
    if prefixes or visit_paths:
        project["sources"] = [
            ...
            and str(source.get("path", "")) not in visit_paths
        ]


def drop_dependency(arguments: argparse.Namespace) -> int:
    """Remove a pin, its calling nodes, and its checkout.

    Missing pieces from a partially removed dependency are tolerated.
    Uncommitted work and unrecorded child commits are rejected before
    changing any files. Authored node code requires the explicit
    ``discard_node_code`` option.
    """
    clone = _here()
    alias = str(arguments.alias)
    complaint = local.package_problem(alias)
    ...
    pins = _objects(project.get("externals"))
    binding_ids = {
        local.definition_child(owner, str(binding.get("id", "")))
        for owner, _, binding in _external_bindings(project)
        if _object(binding.get("external")).get("alias") == alias
    }
    called = any(
        _calls_dependency(node, alias, binding_ids)
        for definition in _subroutine_definitions(project)
        for node in _objects(definition.get("nodes"))
    )
    links = _dependency_gitlinks(clone.root, target)
    sections = _dependency_sections(clone.root, target)
    nested = set(links) - {target}
    nested.update(
        local.external_root(str(pin.get("alias")))
        for pin in pins
        if pin.get("alias") != alias
        and local.external_root(str(pin.get("alias"))).startswith(target + "/")
    )
    if nested:
        raise local.WorkspaceError(
            f"{target} contains other dependencies ("
            f"{', '.join(sorted(nested))}); "
            "drop their exact aliases instead"
        )
    owned = (
        any(pin.get("alias") == alias for pin in pins)
        or binding_ids
        or called
        or links
        or sections
    ...
    if not owned:
        ...
        return 0
    _validate_drop_checkout(checkout, target, links.get(target, ""))
    project["externals"] = [pin for pin in pins if pin.get("alias") != alias]
    dropped = len(pins) - len(project["externals"])
    bindings, callers, directories, visits = _remove_dependency_calls(
        clone, project, alias, binding_ids
    )
    _drop_authored_code(
        clone,
        project,
        directories,
        visits,
        discard=bool(getattr(arguments, "discard_node_code", False)),
    )
    if dropped or bindings or callers or directories or visits:
        (clone.root / "project.json").write_text(
            json.dumps(project, indent=2, ensure_ascii=False) + "\n",
            encoding="utf-8",
        )

    # Keep .git/modules: it may hold the only recoverable copy of child commits.
    removed_files = checkout.exists()
    _quiet_git(clone.root, "submodule", "deinit", "--force", target)
    _quiet_git(clone.root, "rm", "--force", "--quiet", target)
    shutil.rmtree(checkout, ignore_errors=True)
    # Remove any stanza git rm left behind. Keep the empty file: a staged
    # deletion
    # of .gitmodules prevents a subsequent git submodule add in this worktree.
    if modules.is_file():
...
def _record_external(
    ...
    for identifier, candidate in _scoped_subroutine_definitions(project):
        ...
        taken = {
            str(
                child.get(
                    "id",
                    local.definition_leaf(str(child.get("subroutine", ""))),
                )
            )
            for child in _objects(workflows)
        }
        binding_id = _free_id(local.definition_leaf(workflow), taken)
        workflows.append(
            {
                "id": binding_id,
                "name": workflow,
    ...
    return local.definition_child(parent, binding_id)
...
def _free_id(requested: str, taken: set[str]) -> str:
    ...
    for suffix in range(2, 1000):
        ...
        if candidate not in taken:
            return candidate
    raise local.WorkspaceError(f"no free identifier near {requested}")
...
def _rename(arguments: argparse.Namespace) -> int:
    ...
    result = session.credential(
        clone.origin, clone.token, anonymous=True
    ).rename(
        original,
        str(arguments.kind),
        (
    ...
    touched = clone.write(
        cast("dict[str, str | None]", files), expected=original
    )
...
def _token(arguments: argparse.Namespace) -> int:
    ...
    Only a session may manage these: a credential that can mint credentials
    is one that cannot be revoked by revoking it.
    """
    clone = _here()
    owner, name = clone.require_repository()
    address = f"{owner}/{name}"
...
def register(commands: Any) -> None:
    ...
    login = commands.add_parser(
        "login", help="sign in with your GitHub account"
    )
    login.add_argument(
        "origin", nargs="?", help="service origin (or VERDOG_BACKEND_ORIGIN)"
    )
    login.add_argument(
        "--github-token-stdin",
        action="store_true",
        help="read an existing GitHub token from stdin "
        "instead of prompting (used by VS Code)",
    )
    ...
    started.add_argument(
        "directory", nargs="?", help="where to put it (default: here)"
    )
    started.add_argument(
        "--package",
        help="its canonical `<space>.<name>` Python package "
        "(required without a GitHub remote)",
    )
    started.add_argument(
        "--origin", help="the Verdog service, if not the default"
    )
    ...
    describe = commands.add_parser(
        "describe",
        help="read the canonical catalogue metadata for one workflow",
    )
    ...
    publish = commands.add_parser(
        "publish", help="offer a workflow at HEAD for reuse"
    )
    publish.add_argument(
        "--description",
        required=True,
        help="one sentence: what this workflow does",
    )
    publish.add_argument(
        "workflow",
        nargs="?",
    ...
    publish.add_argument(
        "--private",
        action="store_true",
        help="deprecated compatibility flag; visibility "
        "follows the GitHub repository",
    )
    publish.set_defaults(handler=publish_workflow)

    retract = commands.add_parser(
        "retract", help="withdraw an offer you published"
    )
    retract.add_argument(
        "entry", help="the entry id, from `verdog catalogue --json`"
    )
    retract.set_defaults(handler=_retract)

    dropped = commands.add_parser(
        "drop", help="stop depending on a pinned workflow"
    )
    dropped.add_argument("alias", help="the direct dependency alias")
    dropped.add_argument(
        "--discard-node-code",
        action="store_true",
        help="permanently discard authored code and visits "
        "belonging to removed call nodes",
    )
    ...
    imported.add_argument(
        "workflow", help="the published workflow path to call"
    )
    imported.add_argument(
    ...
    bump.add_argument(
        "--commit",
        help="a specific 40-character commit; default is the newest offered",
    )
    bump.set_defaults(handler=bump_dependency)

    rename = commands.add_parser(
        "rename", help="rename a graph entity and its modules"
    )
    rename.add_argument(
        "kind",
        choices=(
    ...
    rename.add_argument(
        "--subroutine",
        help="project-local scope path; required unless "
        "renaming a workflow or subroutine",
    )
    rename.set_defaults(handler=_rename)

    token = commands.add_parser(
        "token", help="tokens for an agent working unattended"
    )
    token.add_argument("action", choices=("create", "list", "revoke"))
    token.add_argument(
        "name", nargs="?", help="a label to create, or an id to revoke"
    )
