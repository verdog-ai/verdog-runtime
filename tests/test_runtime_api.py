...
def update_features(input, state, context, /):
    ...
    try:
        ...
        raise AssertionError(
            "FeatureState replaced ordinary node state"
        )
    if mode == "touch_other":
...
try:
    ...
    raise AssertionError(
        "an executable target accepted an edge without a visit"
    )
try:
    validate_graph(
        replace(
...
events = []
output = output_dir()
result = Dispatcher(execution_handler=events.append).run(
    definition(GRAPH),
    original,
    output_dir=output,
    run_id=RunId("run"),
...
for node_id, node_type in (
    ...
    assert any(
        row[:5] == [".", str(GRAPH.id), node_id, node_type, "1"]
        for row in rows
    )
fired = next(
    event
...
try:
    (
        Dispatcher().run(
            definition(GRAPH), original, output_dir=output_file
        )
    )
except ValueError as error:
...
result = Dispatcher().run(definition(GRAPH), 7, output_dir=output_dir())
...
# Conditions observe sigma, while effects compare sigma with sigma-
# prime.
run(1, POSITIVE.id)
# With no explicit effect, the inferred frame requires the saturated
# value
# to remain unchanged.
run(0, ZERO.id)
""",
...
assert (
    str(caught)
    == "subroutine declared_failure reached its failure port"
)
...
fired = next(
    ...
    for event in events
    if isinstance(event, EdgeExecution)
    and event.edge_id == TO_FAILURE.id
)
...
stack = (output / trace[-1] / "stacktrace.txt").read_text("utf-8")
assert (
    "RuntimeError: subroutine declared_failure reached its failure port"
    in stack
)
...
assert isinstance(result, Success)
# 10 -> worker_a adds one -> worker_b adds two -> collect passes it
# through.
assert result.output == 13
# Sequential, so every node's state and the feature all persist into the
# final
# workflow state. The old assertions here said the opposite -- they
# existed
...
CHILD_MODULE = install_definition_module(
    SubroutineDefinition(graph=CHILD)
)
...
result = Dispatcher(execution_handler=nested_events.append).run(
...
assert any(
    row[1:5]
    == [str(PARENT.id), str(CHILD_NODE.id), "subroutine_call", "1"]
    for row in rows
)
...
assert any(
    row[1:5] == [str(CHILD.id), str(CHILD_WORK.id), "python", "1"]
    for row in rows
)
for name in ("config.md", "stats.md"):
    assert call_reports(nested_output / name) == [
...
recovery_events = []
result = Dispatcher(execution_handler=recovery_events.append).run(
...
child_failures = [
    path
    for path in recovery_trace
    if path.endswith("/child_failure/000001")
]
...
assert [
    ...
    if isinstance(event, NodeExecution)
    and event.node_id == CHILD_FAILURE.id
] == [ExecutionStatus.RUNNING, ExecutionStatus.FAILED]
assert [
    event.status
    for event in recovery_events
    if isinstance(event, NodeExecution)
    and event.node_id == CHILD_NODE.id
] == [ExecutionStatus.RUNNING, ExecutionStatus.SUCCEEDED]
child_events = [
    event
    for event in recovery_events
...
for name in ("config.md", "stats.md"):
    assert call_reports(recovery_output / name) == [
        (child_output / name).resolve()
    ]
    assert call_reports(child_output / name) == []
assert any(
    row[2:5] == ["child_failure", "failure", "1"]
...
failure_paths = [
    ...
    if path.endswith(("/child_failure/000001", "parent_failure/000001"))
]
...
def test_an_operation_the_runtime_does_not_know_is_refused_rather_than_run(
    ...
    """Reject unknown operations before dispatch can mistake them for calls."""
    ...
    _run(
        ...
        # Graph validation is silent about it, which is why the guard has to be
        # in the
        ...
        try:
            ...
            assert (
                "Bespoke is not an operation this runtime executes"
                in str(error)
            )
            assert "[unsupported_operation]" in str(error)
            assert any(
                "Verdog entity: bespoke" in note for note in error.__notes__
            )
        else:
