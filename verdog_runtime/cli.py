...
_InputType: TypeAlias = TypeForm[InputT] | UnionType
...
class WorkflowArguments(Generic[InputT, RuntimeOptionsT]):
    ...
    params: Mapping[ParameterAddress, object] = field(
        default_factory=lambda: _NO_PARAMETERS
    )


def _address_name(address: ParameterAddress, /) -> str:
    project_path, graph_id = address
    parts = list(PurePosixPath(project_path).parts) if project_path != "." else []
    aliases: list[str] = []
    while parts:
        if parts.pop(0) != "external":
            raise ValueError(f"unsupported parameter project path: {project_path}")
        package: list[str] = []
...
def _default_field(value_type: object, /) -> Field[object]:
    if value_type is NoneType:
        ...
        return field()
    return cast(Field[object], field(default=default))
...
def _params_type(
    params_types: Mapping[ParameterAddress, object], /
) -> tuple[type[object], tuple[tuple[str, ParameterAddress], ...]]:
    fields: list[tuple[str, object, Field[object]]] = []
    bindings: list[tuple[str, ParameterAddress]] = []
    names: set[str] = set()
    for index, (raw_address, value_type) in enumerate(params_types.items()):
        address = normalize_parameter_address(raw_address)
        if value_type is NoneType or value_type is object:
            continue
        name = _address_name(address)
        if name in names:
            raise ValueError(f"duplicate parameter command-line address: {name}")
        names.add(name)
        internal = f"p{index}"
        fields.append((internal, _annotation(value_type, name=name), field()))
...
def parse_arguments(
    ...
    /,
    *,
    params_types: Mapping[ParameterAddress, object] = _NO_PARAMETERS,
...
def parse_arguments(
    ...
    *,
    ...
    envelope_fields: list[tuple[str, object, Field[object]]] = []
    envelope_fields.append(
        ("input", _annotation(input_type), _default_field(input_type))
    )
    envelope_fields.append(("params", params_class, field()))
    if runtime_options is not None:
        envelope_fields.append(
            ("runtime", _annotation(runtime_options), _default_field(runtime_options))
        )
    envelope = cast(
        type[object],
        make_dataclass(
    ...
    params_value = getattr(parsed, "params")
    ...
    return WorkflowArguments(
        input=cast(InputT, getattr(parsed, "input")),
        runtime=(
            None
            if runtime_options is None
            else cast(RuntimeOptionsT, getattr(parsed, "runtime"))
        ),
        params=params,
    )
